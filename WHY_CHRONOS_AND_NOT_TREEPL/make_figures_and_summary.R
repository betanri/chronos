...
method_cols <- c(treePL = "treePL_MAE", chronos = "chronos_MAE")
if (has_rt) method_cols <- c(method_cols, RelTime = "reltime_MAE")
...
boxplot(
  ...
  ylab = "log10(MAE)",
  main = "Overall dating error across the legacy exact-root benchmark"
)
...
par(mar = c(8, 6, 4, 2))
mat <- do.call(rbind, lapply(method_names, function(method) bc[[paste0(method, "_mean")]]))
colnames(mat) <- bc$group
rownames(mat) <- method_labels
barplot(
  mat,
  beside = TRUE,
  col = unname(method_colors),
...
legend("topright", legend = method_labels, fill = unname(method_colors), bty = "n")
dev.off()
...
heat_maps <- lapply(method_cols, function(col) {
...
layout(matrix(seq_len(length(method_names) + 1), nrow = 1), widths = c(rep(4, length(method_names)), 1.4))
...
for (i in seq_along(method_names)) {
  ...
  image(het_vals, mu_vals, t(heat), col = cols, zlim = zlim,
        ...
        main = paste0(method_labels[[i]], " mean MAE"))
...
y_all <- unlist(comb[method_names], use.names = FALSE)
...
par(mfrow = c(1, length(method_names)), mar = c(5, 5, 4, 2))
