...
TARGET_TREE_FILE <- "Terapontoidei_PHYLOGRAM_ML_MAIN.tre"
...
REFERENCE_TIME_TREE <- "NO_Neoscorpis_Parascorpis/NO_Eupercaria_SUBSETS_MAIN_ONLY_BUT_RENAMED_AS_SUBSET/Eupercaria_Congru_yesplectoBEAST_ASTRAL_sub1.tree"
...
run_chronos_modelselect <- function(phy, calib) {
  ...
  if (identical(robust$model, "clock") &&
      ...
      (best_nonclock$score - clock_pl$score) >= PLOG_CLOCK_SWITCH_THRESH) {
...
fit <- run_chronos_modelselect(target_tree, calib)
if (is.null(fit$tree)) stop("chronos failed to return a dated tree.")

safe_id <- gsub("[^A-Za-z0-9_]+", "_", target_id)
dated_tree_file <- file.path(OUT_DIR, paste0(safe_id, "_chronos_dated.tre"))
...
write.tree(fit$tree, dated_tree_file)
...
summary_row <- data.frame(
  ...
  chronos_model = fit$model %||% "",
  chronos_lambda = fit$lambda %||% NA_real_,
  chronos_fit_score = fit$score %||% NA_real_,
  chronos_ploglik = fit$ploglik %||% NA_real_,
  chronos_nb_rate_cat = fit$nb_rate_cat %||% NA_integer_,
  selector_note = fit$selector_note %||% "",
  dated_tree_file = dated_tree_file,
  cal_csv_file = cal_csv_file,
  stringsAsFactors = FALSE
)

write.csv(summary_row, summary_file, row.names = FALSE)
saveRDS(list(summary = summary_row, fit = fit, calib = calib, cal_pairs = cal_pairs), rds_file)
saveRDS(list(summary = summary_row), ckpt_file)

msg("Completed: model=", summary_row$chronos_model,
    " lambda=", summary_row$chronos_lambda,
    " score=", format(summary_row$chronos_fit_score, digits = 6))
msg("Saved: ", summary_file)
msg("Saved: ", dated_tree_file)
msg("Saved: ", rds_file)
